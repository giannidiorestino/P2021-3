...
together with the relevant matrix theory. Almost every Probability
textbook discusses Gaussian random variables, for example,
P. Malliavin \cite[Ch.~V]{malliavin:1995} (graduate),
\cite[Ch~16]{jacod|protter:2003} (undergraduate), and S.H. Ross
\cite[Ch.~10]{ross:2010introduction10} (elementary, many editions
...
basic properties of the univariate Gaussian distribution. We urge the
reader to check this material. When needed, extensive asides treat
...
with density
\begin{equation*}
  \gamma(z) = (2\pi)^{-1/2} \euler^{-z^2/2}
\end{equation*}
with respect to the Lebesgue measure space $(\reals,\mathcal B,m)$.

\subsection{Recap: Change of variable formula in $\reals^d$}. See \cite{rudin:1987-3rd} on multivariate differential Calculus. 
Recall that the derivative at $x$ of a differentiable $f \colon \reals^n \to reals ^m$ is a linear mapping $df(x) \colon \reals^n \to reals ^m$. Given respective bases, the linear mapping is expressed by a matrix $m \times n$, the Jacobian matrix. The derivative of the composite function is given by $d g \circ f (x) = dg(f(x) \circ df(x)$. In particular,
\begin{equation*}
  I = d f^{-1} \circ f(x) = d f^{-1} (f(x)) \circ df(x) \quad \Rightarrow \quad df^{-1}(y) = f(f^{-1}(y))^{-1} \ . 
\end{equation*}
